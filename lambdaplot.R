...
plot.a <- function(start,end,by){
	...
	y <- c()
	for(i in 1:length(x)){
		y <- c(y,density.f(x[i],10))
	}
	plot(x,y,xlab="x",ylab="density: f(x)",main="lamda = 10") 
}

plot.b <- function(start,end,by){
	# generating x and setting plot
	x <-seq(start,end,by)
	...
	y1 <- c()
	for(i in 1:length(x)){
		y1 <- c(y1,density.f(x[i],0.5))
	}
	# labda = 1
	y2 <- c()
	for(i in 1:length(x)){
		y2 <- c(y2,density.f(x[i],1))
	}
	# labda = 3
	y3 <- c()
	for(i in 1:length(x)){
		y3 <- c(y3,density.f(x[i],3))
	}
	# labda = 10
	y4 <- c()
	for(i in 1:length(x)){
		y4 <- c(y4,density.f(x[i],10))
	}
